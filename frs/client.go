...
func  (client *FrsClient) FrsVerify(sessionId string,encoded string) (response string){
      src := fmt.Sprintf("src%d",rand.Intn(1000))
      loc := fmt.Sprintf("loc%d",rand.Intn(1000))
        s := fmt.Sprintf("{\"session_id\": \"%s\",\"target_score\" : 0.8, \"request_client\" : \"Test1\",  \"action_enable\": 0,\"source_id\" : \"%s\",\"location\":\"%s\",\"image\":\"%s\"}",
